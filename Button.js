...
function DibujarPetalo(x, y, RadioX, scala, Rotacion, colorPetal, colorBorde, grosorBorde, pasos) {
  const Numero = scala;
  ...
  // Configurar el borde del pétalo
  ctx.strokeStyle = colorBorde;
  ctx.lineWidth = grosorBorde;
  ctx.fillStyle = colorPetal;
  
  // Dibujar el borde y rellenar el pétalo
  ctx.stroke();
  ctx.fill();
    ...
    if (NuevaY < y + AltoTrazo) {
      ...
      function DibujarPetalosTallo() {
        if (CuantosPasos <= Pasos) {
          ...
          DibujarPetalo(500, PetaloY, 15, 2, 300, 'green','black',2,CuantosPasos);
          DibujarPetalo(470, PetaloY2, 15, 2, 300, 'green', 'Black',2,CuantosPasos);
          CuantosPasos++;
  ...
  DibujarTallo();
    ...
    function dibujarSiguientePetalo() {
        if (contadorPetalos <= NumeroPetalos) {
          ...
          DibujarPetalo(x, y, RadioXPetalo, 2, Angulo, 'yellow','Black',2, 100);
          contadorPetalos++;
